...
# w2 = "U62,R66,U55,R34,D71,R55,D58,R83"  # = distance 159
# w1 = "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51"
# w2 = "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"  # = distance 135
...
def f(ws, a):
    ...
    for w in ws:
        ...
        for _ in range(int(w[1:])):
            p[i] += j
            if tuple(p + [not a]) in g:
                min_dist = min(md(p), min_dist)
            g.add(tuple(p + [a]))
...
def f2(ws, a):
    ...
    for w in ws:
        ...
        for _ in range(int(w[1:])):
            ...
            n += 1
            if tuple(p + [not a]) in g:
                min_steps = min(g[tuple(p + [not a])] + n, min_steps)
            g[tuple(p + [a])] = n
