...
def draw():
    ...
    for y in range(extent[2] - 1, extent[3] + 1):
        row = []
        for x in range(extent[0] - 1, extent[1] + 1):
            # print(x, y)
            if (x, y) in walls:
                row.append("#")
            elif (x, y) in oxygen:
...
DIRS = {1: "north", 2: "south", 3: "west", 4: "east"}
...
def visit(pos, from_direction=None, steps=1):
    global robot, visited, walls, oxygen_pos
    visited.add(pos)
    ...
    for pos in oxygen:
        for direction in [1, 2, 3, 4]:
            if direction == 1:
                ...
                candidate_pos = (pos[0] + 1, pos[1])
            if candidate_pos not in walls and candidate_pos not in oxygen:
                expansion.add(candidate_pos)
    oxygen |= expansion
    if not expansion:
        break
    n_minutes += 1
